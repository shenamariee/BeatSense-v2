...
# ---------------- CONFIG ----------------
WORK_DIR = "wfdb_files"
LOGO_FILENAME = "BeatSense Logo.jpg"
...
if "patient_info" not in st.session_state:
    st.session_state.patient_info = {"name": "", "age": 0, "gender": "Not specified"}
...
def save_uploaded_files(files, dest_dir):
    saved = []
    for f in files:
        with open(os.path.join(dest_dir, f.name), "wb") as out:
            out.write(f.read())
        saved.append(f.name)
    return saved
...
with st.sidebar:
    if os.path.exists(LOGO_FILENAME):
        ...
        st.markdown("## ❤️ BeatSense")

    st.markdown("### Current Step")
    st.info(st.session_state.page)

    st.markdown("---")
    st.markdown("Academic demo only – not for clinical use.")

# ---------------- HOME ----------------
def show_home():
    st.title("BeatSense")
    st.write("ECG Rhythm Detection System (Academic Demo)")
    if st.button("Proceed to Working Principle", key="home_btn"):
...
def show_working_principle():
    ...
    st.write("""
1. ECG files are uploaded in WFDB format  
2. R-peaks are detected  
3. Beat features are extracted  
4. Machine learning performs classification  
5. Results are displayed  
""")
    if st.button("Proceed to Terms", key="wp_btn"):
...
def show_terms():
    ...
    if st.button("Accept Terms", key="terms_accept"):
...
def show_patient_info():
    ...
    if st.button("Proceed to ECG Upload", key="to_ecg"):
...
def show_ecg_analysis():
    ...
    # ✅ ✅ ✅ UPLOADER IS ONLY HERE NOW
    uploaded_files = st.file_uploader(
        "Upload WFDB Files (.hea, .dat, .atr)",
        type=["hea", "dat", "atr"],
    ...
    bases = get_base_names(WORK_DIR)
    if not bases:
        st.info("Upload ECG records first.")
        return
