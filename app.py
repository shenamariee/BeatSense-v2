...
# --- Full ECG Upload & Analysis page function ---

def ecg_upload_and_analysis():
    ...
    if run_button:
        ...
        for i in range(min(20, len(beat_features))):
            ...
            beat_rows.append({
                "Beat #": i+1,
                "ML Label": ml_label,
                "Annotation": labels[i] if i < len(labels) else "N/A",
                "HR (bpm)": f"{60/rr[i]:.1f}",
                "Sequence Rhythm": tachy_results[i] if i < len(tachy_results) else "N/A"
            })
        st.table(pd.DataFrame(beat_rows))

        # --- Save simple summary string for Results page ---
        dominant_rhythm = "Normal"
        counts = overall_summary
        if counts.get("Bradycardia", 0) > counts.get("Normal", 0) and counts.get("Bradycardia", 0) > counts.get("Tachycardia", 0):
            dominant_rhythm = "Bradycardia"
        elif counts.get("Tachycardia", 0) > counts.get("Normal", 0):
            dominant_rhythm = "Tachycardia"
        elif counts.get("Normal", 0) > 0:
            dominant_rhythm = "Normal"
        else:
            dominant_rhythm = "Abnormal"

        st.session_state['last_summary'] = dominant_rhythm
...
# --- New Results page function with your friendly messages ---
...
def main():
    ...
    page = st.sidebar.radio("Go to", ["ECG Upload & Analysis", "Results"])

    if page == "ECG Upload & Analysis":
        ecg_upload_and_analysis()
    elif page == "Results":
        show_results()
